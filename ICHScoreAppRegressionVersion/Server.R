...
shinyServer(function(input, output) {
  ...
  ROC <- reactive({
    ...
    optimal.cutpoint.Youden<-optimal.cutpoints(X = "rev.ich", status = "Outcome", tag.healthy = 0, 
                                               methods = "Youden", data = my.df)
    
    
    
    return(list (optimal.cutpoint.Youden,optimal.cutpoint.Youden$Youden$Global$optimal.cutoff))
  ...
  LR <- reactive({
    ...
    modelNnet <- train(Outcome ~ Age + Sex + MAP +   
                            RBS + GCS + tentorium + VE + Volume , data = trainset,
    ...
    results <- resamples(list( LR = modelLR, RF = modelRF ))
    e9 = summary(results)
    ...
    temps$Nnet <- predict(modelNnet, newdata = temps)
    ...
    e6= print ( " Predictions made by NeuralNetwork machine Algorithm  " )
    e8 = print ( " ICH score based on input and Predictions made by it  " )
    
    p = list(modelLR,modelRF,modelNnet,e9,e1,temps$LR,e7,temps$RF,e6,temps$Nnet,e8,k,newich) 
